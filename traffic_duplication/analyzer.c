...
unsigned long long expected_time = 0;
unsigned long long prev_sequence_nr = 0;
...
int record_traffic_starvation() {
  if (prev_sequence_nr == 0) {
    return;
  }

  char *name = output_file_name();
  FILE *f = fopen(name, "ab+");
  int rv = fprintf(f, "%llu %u %llu %u\n", count, missed, out_of_order_count, out_of_order);
  fclose(f);
  free(name);
...
int receive_packet()
{
  ...
  // Out of order
  if (prev_sequence_nr == 0) {
    prev_sequence_nr = sequence_number;
  } else {
    int diff = prev_sequence_nr - sequence_number;
    int sgn = (diff > 0) - (diff < 0);
    if (sgn * abs(diff) < sgn * PACKET_OUT_OF_ORDER_THRESHOLD) {
      out_of_order_count++;
    }
    prev_sequence_nr = sequence_number;
  }
